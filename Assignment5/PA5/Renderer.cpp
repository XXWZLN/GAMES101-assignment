...
void Renderer::Render(const Scene &scene)
{
    ...
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            // float n = scene.height / scale;
            float x = 2 * scale * imageAspectRatio / scene.width * (i + 0.5) - scale * imageAspectRatio;
            float y = - 2 * scale / scene.height * (j + 0.5) + scale;
            float n = -scene.height/2;
